...
function onCreateRoomClick() {
	...
	req.onreadystatechange = function () {
		if (req.readyState == XMLHttpRequest.DONE && req.status == 200) {
			...
			function onBlueLinkClick(event: any) {
				...
				if (event.altKey) {
					window.open("/" + (<HTMLInputElement>document.querySelector("#blueLink")).innerHTML);
				} else {
					(<HTMLInputElement>document.querySelector("#blueLink")).innerHTML = "link copied";
					setTimeout(function () {
						(<HTMLInputElement>document.querySelector("#blueLink")).innerHTML = blueLink;
					}, 200);
				}
			...
			function onRedLinkClick(event: any) {
				var copyLink = document.createElement("input");
				...
				document.body.removeChild(copyLink);
				if (event.altKey) {
					window.open("/" + (<HTMLInputElement>document.querySelector("#redLink")).innerHTML);
				} else {
					(<HTMLInputElement>document.querySelector("#redLink")).innerHTML = "link copied";
					setTimeout(function () {
						(<HTMLInputElement>document.querySelector("#redLink")).innerHTML = redLink;
					}, 200);
				}
			...
			function onSpyLinkClick(event: any) {
				var copyLink = document.createElement("input");
				...
				document.body.removeChild(copyLink);
				if (event.altKey) {
					window.open("/" + (<HTMLInputElement>document.querySelector("#spyLink")).innerHTML);
				} else {
					(<HTMLInputElement>document.querySelector("#spyLink")).innerHTML = "link copied";
					setTimeout(function () {
						(<HTMLInputElement>document.querySelector("#spyLink")).innerHTML = spyLink;
					}, 200);
				}
			...
			(<HTMLInputElement>document.querySelector("#hint")).innerHTML = "Click on the room code to copy the link!<br>Then paste it to your friends!<br>Then click home and join the room!<br><br>Option-Click on the room code to open room in a new window!";
